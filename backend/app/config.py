    ...
    def validate_jwt_secret(cls, v):
        if not v or v.startswith("change-this"):
            ...
            if is_mock:
                ...
                return ""
            # Derive a stable secret from supabase_url so tokens survive restarts
            import hashlib
            supabase_url = os.getenv("SUPABASE_URL", "https://placeholder.supabase.co")
            derived = hashlib.sha256(f"cvber-jwt-{supabase_url}".encode()).hexdigest()
            logger.warning(
                "JWT_SECRET not set — derived stable secret from SUPABASE_URL. "
