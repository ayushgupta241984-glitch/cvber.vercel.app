...
async def register(request: Request, body: RegisterRequest):
    try:
        ...
        if is_mock_mode():
            # Mock mode: log but still create tokens (for local dev only)
            logger.warning(f"MOCK MODE: Creating mock user {body.email} — not connected to Supabase")
        else:
            try:
                user_attributes = {
                    "email": body.email,
                ...
                if admin_response and admin_response.user:
                    ...
                    raise HTTPException(status_code=400, detail="Registration failed — Supabase returned no user")
                ...
                if "already registered" in err_msg.lower():
                    ...
                    raise HTTPException(status_code=400, detail="Password is too weak. Use at least 8 characters with mixed case and numbers.")
                else:
                    logger.error(f"Registration failed: {err_msg[:200]}")
...
async def login(request: Request, body: LoginRequest):
    try:
        ...
        if is_mock_mode():
            logger.warning(f"MOCK MODE: Creating mock session for {body.email} — not connected to Supabase")
        else:
            try:
                ...
                if "Invalid login credentials" in err_msg:
                    raise HTTPException(status_code=401, detail="Invalid email or password")
                logger.error(f"Login failed: {err_msg[:200]}")
