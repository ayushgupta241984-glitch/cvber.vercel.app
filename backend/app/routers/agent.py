...
async def _cross_search_images(source_bytes: bytes, result_urls: list, max_pages: int = 5, max_imgs_per_page: int = 10) -> list:
    ...
    try:
        ...
        logger.warning("bs4 not installed — skipping cross-search")
        return []
    import re, httpx
...
async def _compare_images(source_bytes: bytes, candidate_url: str) -> dict:
    ...
    try:
        ...
        async with httpx.AsyncClient(timeout=15, follow_redirects=True) as client:
            ...
            # Primary: ORB feature matching (requires opencv-python)
            src_kp_count = 0
            cnd_kp_count = 0
            try:
                import cv2
            ...
            # SSIM: secondary comparison, used as confirmation (requires scikit-image)
            try:
                target_size = (256, 256)
            ...
            # Only apply low-texture cap if ORB actually ran (cv2 available)
            orb_ran = src_kp_count > 0 or cnd_kp_count > 0
            if orb_ran and (src_kp_count < 200 or cnd_kp_count < 200):
...
async def get_notifications(current_user: dict = Depends(get_current_user)):
    """Get agent notifications (new copies found, monitoring alerts)."""
    try:
        supabase = await _get_supabase()
        result = supabase.table("agent_notifications") \
            .select("*") \
...
async def get_investigations(current_user: dict = Depends(get_current_user)):
    """Get agent investigations (find_copies history with findings)."""
    try:
        supabase = await _get_supabase()
        result = supabase.table("agent_investigations") \
            .select("*") \
            .eq("user_id", current_user["id"]) \
            .order("created_at", desc=True) \
            .limit(50) \
            .execute()
        return {"investigations": result.data or []}
    except Exception as e:
        logger.warning(f"get_investigations error: {e}")
        return {"investigations": []}


@router.post("/monitor/run")
async def run_monitor(current_user: dict = Depends(get_current_user)):
    """Trigger monitoring scan for all indexed files."""
    try:
        from app.services.monitor_worker import run_monitoring_cycle
        result = await run_monitoring_cycle(current_user["id"])
        return result or {"status": "completed", "alerts": 0}
    except Exception as e:
        logger.warning(f"run_monitor error: {e}")
...
async def agent_chat(
    ...
    try:
        ...
        logger.error(f"Agent chat error: {type(e).__name__}: {e}", exc_info=True)
        error_detail = str(e)
        # If NIM failed and Groq is available, retry with Groq
        if has_nim and has_groq and ("nvidia" in error_detail.lower() or "nim" in error_detail.lower()):
            try:
                logger.info("NIM failed, retrying with Groq fallback")
                client = AsyncOpenAI(
                    api_key=settings.groq_api_key,
                    base_url="https://api.groq.com/openai/v1",
                )
                ...
                error_detail = f"NIM: {error_detail}; Groq fallback: {fallback_err}"
        raise HTTPException(status_code=500, detail=f"Agent chat failed: {error_detail[:300]}")
