...
export default function HowItWorksPage() {
    return (
        <div className="min-h-screen bg-black text-white selection:bg-white/[0.06]/25 overflow-x-hidden">
            ...
            <section className="relative pt-40 pb-20 px-8 md:px-16 overflow-hidden">
                ...
                <div className="max-w-7xl mx-auto text-center relative z-10">
                    ...
                    <motion.h1 initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}
                        ...
                        Security in<br /><span className="text-white/60">Three Steps.</span>
                    </motion.h1>
                ...
                <div className="max-w-7xl mx-auto">
                    <div className="grid md:grid-cols-3 gap-12 md:gap-16">
                        {[
                            ...
                            >
                                <div className="text-[10rem] font-bold text-white/[0.02] absolute -top-20 -left-8 pointer-events-none select-none">
                                    {item.step}
                                </div>
                                <div className="relative z-10">
                                    <div className="w-16 h-16 rounded-full border border-white/[0.15]/25 flex items-center justify-center mb-10 group-hover:border-white/[0.15]/50 transition-colors">
                                        <item.icon className="w-8 h-8 text-white/60" />
                                    </div>
                                    <h3 className="text-3xl font-bold text-white mb-6">{item.title}</h3>
                                    <p className="text-white/40 font-sans text-base leading-relaxed">{item.desc}</p>
                                </div>
                            </motion.div>
                        ))}
                    </div>
            ...
            {/* Tech Deep Dive */}
            <section className="py-24 md:py-32 px-8 md:px-16 border-y border-white/[0.08] bg-[#050505]/50">
                <div className="max-w-7xl mx-auto">
                    <div className="grid md:grid-cols-2 gap-12 md:gap-24 items-center">
                        <motion.div initial={{ opacity: 0, x: -30 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }}>
                            <span className="tag mb-6 block">Technology</span>
                            <h2 className="text-4xl md:text-6xl font-bold text-white mb-8 leading-tight">
                                Built for the<br /><span className="text-white/60">AI Era.</span>
                            </h2>
                            <p className="text-white/40 font-sans text-base leading-relaxed mb-10">
                            ...
                            <div className="space-y-4">
                                <div className="bg-[#0a0a0a] border border-white/[0.08] p-6 md:p-8 flex gap-6">
                                    <div className="shrink-0 w-10 h-10 rounded-full border border-white/[0.15]/20 flex items-center justify-center">
                                        <ShieldCheck className="w-5 h-5 text-white/60" />
                                    </div>
                                    <div>
                                        <h4 className="font-sans text-sm font-bold text-white mb-1 uppercase tracking-wide">C2PA Standard Compliance</h4>
                                        <p className="text-white/40 text-xs font-sans">Industry-standard protocols used by Adobe and Microsoft for universal verification.</p>
                                    </div>
                                </div>
                                <div className="bg-[#0a0a0a] border border-white/[0.08] p-6 md:p-8 flex gap-6">
                                    <div className="shrink-0 w-10 h-10 rounded-full border border-white/[0.15]/20 flex items-center justify-center">
                                    ...
                                    <div>
                                        <h4 className="font-sans text-sm font-bold text-white mb-1 uppercase tracking-wide">Neural Monitoring</h4>
                                        <p className="text-white/40 text-xs font-sans">Detection of fingerprints even if the image is cropped, filtered, or heavily compressed.</p>
                                    </div>
                                </div>
                            </div>
                        </motion.div>
                        ...
                        <motion.div
                            ...
                            viewport={{ once: true }}
                            className="relative aspect-square rounded-3xl bg-[#0a0a0a] border border-white/[0.08] overflow-hidden flex items-center justify-center"
                        >
                            <div className="absolute inset-0 bg-gradient-radial from-white/[0.03] to-transparent" />
                            <div className="absolute inset-0 opacity-[0.03]">
                            ...
                            {[1, 2, 3, 4].map((r) => (
                                <motion.div
                                    ...
                                    className="absolute rounded-full border border-white/[0.15]/20"
                                    style={{ inset: `${r * 10}%` }}
                                />
                            ))}
                            {/* Node dots */}
                            {[
                                ...
                                <motion.div
                                    ...
                                    className="absolute w-2 h-2 bg-white/[0.06] rounded-full"
                                    style={{ left: pos.x, top: pos.y, boxShadow: '0 0 6px rgba(201, 169, 98, 0.4)' }}
                                />
                            ))}
                            <div className="relative z-10 text-center">
                                <div className="text-[9px] font-bold text-white/60/50 uppercase tracking-[0.3em] font-sans mb-2">Neural Grid Active</div>
                                <div className="text-2xl md:text-3xl font-bold text-white/10">Monitoring</div>
                            </div>
                        </motion.div>
                    </div>
                </div>
