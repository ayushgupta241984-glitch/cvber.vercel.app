...
export default function VerifyLandingPage() {
    ...
    return (
        <div className="min-h-screen bg-black text-white selection:bg-white/[0.03]/25 overflow-x-hidden">
            ...
            <div className="fixed inset-0 z-0 pointer-events-none">
                ...
                <motion.div animate={{ rotate: 360 }} transition={{ duration: 90, repeat: Infinity, ease: "linear" }}
                    className="absolute top-[-20%] right-[-10%] w-[800px] h-[800px] bg-white/[0.03]/[0.02] blur-[120px] rounded-full" />
            </div>
            ...
            <div className="relative z-10 max-w-4xl mx-auto pt-[20vh] px-8 md:px-16 pb-32">
                ...
                <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="text-center mb-20">
                    ...
                    <h1 className="text-5xl md:text-8xl font-bold text-white mb-8 leading-tight">
                        Identity<br /><span className="text-white/60">Verified.</span>
                    </h1>
                ...
                <motion.div initial={{ opacity: 0, scale: 0.97 }} animate={{ opacity: 1, scale: 1 }} transition={{ delay: 0.2 }}
                    className={`relative p-[1px] rounded-3xl transition-all duration-700 ${isFocused ? "bg-gradient-to-b from-white/40 via-white/10 to-transparent" : "border-white/[0.08]"}`}>
                    <div className="bg-[#0a0a0a] rounded-3xl p-10 md:p-16 relative overflow-hidden">
                        ...
                        <div className="relative z-10 space-y-10">
                            <div className="flex items-center gap-4">
                                ...
                                <h2 className="text-2xl font-bold text-white">Input Cipher</h2>
                            </div>
                            ...
                            <div className="space-y-4">
                                <textarea
                                    ...
                                    className="w-full px-6 py-6 bg-black/50 border border-white/[0.08] rounded-2xl text-sm font-mono focus:outline-none focus:border-white/[0.15]/50 transition-colors resize-none h-28 text-white placeholder-white/30"
                                />
                                <AnimatePresence>
                                    {error && (
                            ...
                            {/* Steps */}
                            <div className="grid md:grid-cols-3 gap-8 pt-8 border-t border-white/[0.08]">
                                {[
                                    ...
                                    <div key={i} className="space-y-1">
                                        <span className="text-[9px] font-bold text-white/40/50 uppercase tracking-ultra-wide font-sans">{step.label}</span>
                                        <p className="text-xs font-bold text-white/40 font-sans">{step.text}</p>
                                    </div>
                                ))}
                            </div>
                        </div>
                ...
                <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.4 }} className="mt-20">
                    <div className="flex items-center gap-2 mb-6 px-4">
                        <Shield className="h-3 w-3 text-white/40/50" />
                        <h3 className="text-white/40/50 font-bold text-[9px] uppercase tracking-ultra-wide font-sans">Recent Identifications</h3>
                    </div>
                ...
                <p className="text-center text-white/40/20 text-[8px] font-bold uppercase tracking-[0.5em] mt-32 leading-relaxed font-sans">
    ...
    if (proofs.length === 0) {
        return (
            <div className="py-10 text-center">
                <p className="text-white/40/30 text-[10px] font-bold uppercase tracking-widest font-sans">
        ...
        <div className="space-y-3">
            {proofs.slice(0, 5).map((proof: any) => (
                <button key={proof.asset_hash} onClick={() => onSelect(proof.asset_hash)}
                    className="w-full p-5 bg-[#0a0a0a] border border-white/[0.08] hover:bg-[#0a0a0a]/80 text-left transition-all group flex items-center justify-between">
                    <div>
                        <p className="text-white group-hover:text-white/60 font-bold text-sm truncate mb-1 transition-colors font-sans">{proof.asset_name}</p>
                        <p className="text-white/40/50 font-mono text-[10px] truncate max-w-[200px] md:max-w-md">{proof.asset_hash}</p>
                    </div>
                    <ArrowRight className="w-4 h-4 text-white/40/30 group-hover:text-white/60 transition-colors" />
                </button>
            ))}
        </div>
    );
